...
const EMAIL_SETTING_KEYS = {
  ...
  smtpPass: "email.smtp.pass",
  smtpFrom: "email.smtp.from",
  defaultRecipients: "email.default.recipients",
  htmlTemplates: "email.html.templates",
  htmlDefaultTemplateId: "email.html.default_template_id"
};
...
function normalizeTemplateRecord(input, index = 0) {
  ...
  const subject = String(source.subject || "").trim() || "Vextrom SpecFlow";
...
async function saveEmailDefaultRecipients(defaultRecipientsRaw) {
...
async function saveEmailHtmlTemplate({ templateId = "", name, subject, html, setAsDefault = false }) {
...
async function setDefaultEmailHtmlTemplate(templateId) {
  const normalizedId = String(templateId || "").trim();
  if (!normalizedId) {
...
async function deleteEmailHtmlTemplate(templateId) {
  const normalizedId = String(templateId || "").trim();
  if (!normalizedId) {
    const err = new Error("Modelo de e-mail invalido.");
    err.statusCode = 422;
