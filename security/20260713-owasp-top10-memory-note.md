...
Riscos abertos prioritários: auditoria npm do backend com 6 altas, 19 moderadas e 2 baixas; frontend com 2 baixas; CSP ainda usa `unsafe-inline`; contêiner de app adiciona `SYS_ADMIN`; imagens auxiliares usam `latest`; falta telemetria centralizada de eventos de segurança e testes dinâmicos de autorização por objeto.

Validações concluídas: `node --check` nos três JS alterados, 9 testes de Module Spec, build do frontend, validação do Compose de produção, teste de rejeição/aceitação de segredos em produção e `git diff --check` planejado para o fechamento.
